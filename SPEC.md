...
Architecture (short):
In-process FluidAudio CoreML (NVIDIA Parakeet TDT v3 on the Apple Neural Engine). Capture grows a live sample stream. A serial segmenter on `ultrawhisper.work` closes a chunk on ~2s of trailing silence or a 15s cap and decodes that slice while talking. Second Option+Space finishes the tail, pastes via clipboard + Cmd+V, then `Thread.sleep(0.35)` restores the clipboard. SwiftPM build. No sherpa, no websocket, no Proxy.swift.

Hotkey: tap `recordHotkey` starts, tap again stops and pastes. Do not rewrite `~/.config/ultrawhisper/config.json`. Change `Config.defaults.recordHotkey` to `alt+space` for new installs only. Cleanup chord and Esc-discard stay as they are.

Stages:
- [x] 1. Decode while talking. Capture grows a live sample stream. A serial segmenter on `ultrawhisper.work` closes a chunk on ~2s of trailing silence (with a voiced-audio minimum so noise does not fire) or a 15s cap, whichever first, and decodes that slice. Join segment texts with spaces. Second Option+Space closes the tail (drop a trailing unvoiced bit shorter than ~250ms), waits for the in-flight queue, pastes the joined text, hides the card. If any segment throws or returns empty when the audio was voiced, fall back once to a full-buffer `transcribe(samples)` so a take is never lost. Esc discard still drops the take and cancels queued work.
...
- [x] 3. In-process FluidAudio CoreML (Parakeet TDT, Apple Neural Engine). Drop the sherpa websocket server, `Proxy.swift` loopback, and `sandbox-exec`. Same tap-toggle, same segmenter. `build.sh` becomes SwiftPM (or an equivalent that still produces `/Applications/UltraWhisper.app` signed with "Postit Dev"). Whisper fallback stays for when Parakeet files are missing.
